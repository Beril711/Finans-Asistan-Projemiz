...
export default function InvestingScreen() {
  ...
  // assetsRef her zaman guncel assets'i tutsun (closure sorunu icin)
  useEffect(() => {
  ...
  const fetchAllData = useCallback(async () => {
    try {
      ...
      if (results[0].status === 'fulfilled') {
        setPortfolio(results[0].value.data);
        anySuccess = true;
      }
      ...
      if (!anySuccess) {
        ...
        const msg = firstError?.reason?.message || 'Sunucuya baglanilamadi';
        setErrorMsg(`Veri yuklenemedi: ${msg}`);
      }
    } catch {
      setErrorMsg('Sunucuya baglanilamadi. Backend calistigini kontrol edin.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  // Periyodik fiyat guncellemesi - backend simülasyonu (her 3 saniye)
  useEffect(() => {
    const refreshPrices = async () => {
      try {
        ...
        if (results[0].status === 'fulfilled') {
          ...
          if (Array.isArray(newAssets) && newAssets.length > 0) {
            setAssets(newAssets.map((a: Asset) => {
              const oldPrice = oldPrices[a.id];
              const newPrice = Number(a.current_price);
              return {
                ...a,
                priceChange: oldPrice ? (newPrice > oldPrice ? 'up' : newPrice < oldPrice ? 'down' : undefined) : undefined,
              };
            }));
          }
        }
        ...
        // silent - periyodik guncelleme hatasi kritik degil
      }
    };
  ...
  const handleReset = () => {
    Alert.alert(
      'Portfoyu Sifirla',
      'Tum yatirimlariniz silinecek ve bakiyeniz 10.000 TL olarak sifirlanacak. Emin misiniz?',
      [
        { text: 'Vazgec', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Hata', `Portfoy sifirlanamadi. ${detail}`);
            }
          },
        },
      ]
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      Alert.alert('Hata', 'Bakiye yuklenemedi.');
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.balanceRow}>
          ...
          <Text style={styles.balanceValue}>
            {formatMoney(balance + totalHoldingsValue)} TL
          </Text>
        </View>
        ...
        <View style={styles.headerButtons}>
          <TouchableOpacity style={styles.depositButton} onPress={() => setDepositModalVisible(true)}>
            ...
            <Text style={styles.resetText}>Bakiye Yukle</Text>
          </TouchableOpacity>
      ...
      >
        ...
        {/* Active Tab Content */}
        {activeTab === 'market' && (
          <MarketTab assets={assets} onTradeComplete={fetchAllData} />
        )}
